...
class AjaxUploader {
    ...
    function __construct(array $allowedExtensions = null, $sizeLimit = null){
        if($allowedExtensions===null) { $allowedExtensions = "png,gif,jpg,jpeg"; }
        if($sizeLimit===null) { $sizeLimit = 1048576; }
    ...
    function handleUpload($uploadFilename, $replace = false){
        if(!is_writable(dirname($uploadFilename))){ return array('error' => ("Server error. Upload directory isn't writable.")); }
        if(!$this->_file){ return array('error' => ('No files were uploaded.')); }
        $size = $this->_file->getSize();
        if($size == 0) { return array('error' => ('File is empty')); }
        if($size > $this->_sizeLimit) { return array('error' => ('File is too large')); }
        ...
        if(($this->_allowedExtensions && stripos($this->_allowedExtensions, strtolower($ext))===false) ) {
            ...
            return array('error' => sprintf(('File has an invalid extension, it should be one of %s.'), $this->_allowedExtensions));
        }
        ...
        if($this->_file->save($uploadFilename)){
            ...
            $files[$_POST['qquuid']] = $uuid['basename'];
...
class AjaxUploadedFileForm {
    private $_file;
    function __construct() { $this->_file = $_FILES['qqfile']; }
