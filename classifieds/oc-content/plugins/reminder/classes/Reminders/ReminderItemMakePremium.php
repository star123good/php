...
class ReminderItemMakePremium extends ReminderTest{
    public function __construct($reminder = array()) {
        parent::__construct($reminder);
        $this->title = __('Make you ad premium', 'reminder');
        $this->description = __('Sends a custom email to non premium ads, that has been created since more than X days ago from publication date.', 'reminder');
        $this->_set_requirement(ER_ITEM_UNBLOCKED);
        $this->_set_requirement(ER_ITEM_VALIDATED);
        $this->_set_requirement(ER_ITEM_NO_PREMIUM);
        $this->_set_requirement(ER_ITEM_NO_SPAM);
    ...
    error_log($this->reminder->last_check. "   ". $d_end);
        $aEmails = ModelReminder::newInstance()->getNewItemsNoPremium($this->reminder->last_check, $d_end);
