    ...
    class CWebRegister extends BaseModel
    {
        ...
        function doModel()
        {
            switch( $this->action ) {
                ...
                break;
		case('private_register'):       //register user
                                        $this->doView('private-register.php');
                break;
                ...
                break;
		case('private_register_post'):  //register user
                                        osc_csrf_check();
                                        if( !osc_users_enabled() ) {
                                            osc_add_flash_error_message( _m('Users are not enabled') );
                                            $this->redirectTo( osc_base_url() );
                                        ...
                                        if($banned==1) {
                                            osc_add_flash_error_message( _m('Your current email is not allowed'));
                                            $this->redirectTo(osc_private_register_account_url());
                                        } else if($banned==2) {
                                            osc_add_flash_error_message( _m('Your current IP is not allowed'));
                                            $this->redirectTo(osc_private_register_account_url());
                                        }
                                        ...
                                        require_once LIB_PATH . 'osclass/UserActions.php';
                                        $userActions = new UserActions(false);
                                        $success = $userActions->add(false); // validate Recaptcha -> false
                                        if($success==1) {
                                            osc_add_flash_ok_message( _m('The user has been created. An activation email has been sent'));
                                            $this->redirectTo(osc_base_url());
                                        } else if($success==2) {
